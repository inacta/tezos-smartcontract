...
const { b58decode, validateAddress, ValidationResult } = require('@taquito/utils');

// Convert an integer to a byte array following the
// Tezos serialization standard
function packInteger(input) {
 ...
 */
function packFourTupleAsLeftBalancedPairs(nonce, minutes, activities, recipients) {
...
function packAddressSet(input) {
    if (!(Array.isArray(input))) {
        throw Error("Input must be of type array");
    }

    // Sort addresses, sincea set is represented as a sorted sequence internally in Michelson.
...
function packString(input) {
    ...
    var res = new Uint8Array(input.length + 5);

    // ASCII encode string
    for (let i = 0; i < input.length; i++) {
        const element = input[i];
        res[i + 5] = element.charCodeAt(0);
    }

    // add size indication
    var SizeInHex = input.length.toString(16).padStart(8, '0');
    for (let i = 0; i < 4; i++) {
        res[i + 1] = parseInt(SizeInHex.substring(2 * i, 2 * i + 2), 16);
    }

    // add type indication for string = 0x01
    res[0] = 1;
