...
contract("fa2_wl", (_accounts) => {
  ...
  describe("transfer and balances", () => {
    ...
    it(`should transfer 1 token from Alice to Bob`, async () => {
      ...
      expect(accountAliceAfter.balance.isEqualTo(accountAliceBefore.balance)).to
        .be.true;
      expect(accountBobAfter.balance.isEqualTo(accountBobBefore.balance)).to.be
        .true;
    ...
    it(`should not transfer tokens from Alice to Bob when Alice's balance is insufficient`, async () => {
      ...
      expect(
        accountAliceAfter.balance.isEqualTo(accountAliceBefore.balance.minus(1))
      ).to.be.true;
      expect(
        accountBobAfter.balance.isEqualTo(accountBobBefore.balance.plus(1))
      ).to.be.true;
